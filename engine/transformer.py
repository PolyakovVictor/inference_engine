...
class Attention:
    def __init__(self, cfg: ModelConfig) -> None:
        ...
        self.cache_k: Tensor = Tensor.zeros(1, self.max_context, self.n_kv_heads, self.head_dim).contiguous().realize()
        self.cache_v: Tensor = Tensor.zeros(1, self.max_context, self.n_kv_heads, self.head_dim).contiguous().realize()
    ...
    def __call__(self, x: Tensor, start_pos: int = 0, mask: Tensor | None = None) -> Tensor:
        ...
        self.cache_k.assign(self.cache_k.shrink((None, (0, start_pos), None, None)).cat(k, dim=1).pad((None, (0, self.max_context - start_pos - seq_len), None, None)))
        self.cache_v.assign(self.cache_v.shrink((None, (0, start_pos), None, None)).cat(v, dim=1).pad((None, (0, self.max_context - start_pos - seq_len), None, None)))
...
class TransformerBlock:
    ...
    def reset_cache(self,) -> None: self.self_attn.reset_cache()
